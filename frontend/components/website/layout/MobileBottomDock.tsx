...
export default function MobileBottomDock() {
  ...
  return (
    <div className='fixed bottom-0 left-0 right-0 z-50 block lg:hidden bg-surface/95 backdrop-blur-md border-t border-border shadow-[0_-4px_20px_rgba(113,49,200,0.12)] select-none'>
      <div className='flex items-center justify-around h-16 px-1 max-w-md mx-auto relative'>
        ...
        >
          ...
          <span className='text-[10px] font-sans font-semibold tracking-tight'>Raffles</span>
        </Link>

        {/* Tab 3: Center Context Switcher Button (Public Website <-> Dashboard) */}
        <Link
          href={isDashboardRoute ? '/' : isAuthenticated ? dashboardHome : '/login'}
          className='flex flex-col items-center justify-center relative -top-3.5 shrink-0 group px-1'
        >
          <div
            className={cn(
              'w-12 h-12 rounded-full flex items-center justify-center shadow-[0_4px_14px_rgba(113,49,200,0.35)] transition-all duration-300 transform group-active:scale-95 border-2 border-white',
              isDashboardRoute
                ? 'bg-accent-bg text-text-brand border-border-medium'
          ...
          <span className='text-[9px] font-sans font-bold uppercase tracking-wider text-text-brand mt-0.5'>
            {isDashboardRoute ? 'Public' : 'Dashboard'}
          </span>
        </Link>
        ...
        <Link
          href={isAuthenticated ? '/dashboard/user/profile' : '/login'}
          className={cn(
            'flex flex-col items-center justify-center flex-1 h-full py-1 text-center transition-all duration-200',
            pathname.startsWith('/dashboard/user') || pathname === '/login'
              ? 'text-text-brand font-bold scale-105'
              : 'text-text-muted hover:text-text-primary',
          )}
        >
