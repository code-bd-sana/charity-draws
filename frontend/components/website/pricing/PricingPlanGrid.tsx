...
export default function PricingPlanGrid({ billingCycle }: PricingPlanGridProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 items-stretch justify-center gap-6 lg:gap-8 w-full max-w-6xl mx-auto px-4 md:px-0">
      {PRICING_PLANS.map((plan) => {
        const dbPlan = dbPlans?.find(
          (p) => p.name.toLowerCase() === plan.name.toLowerCase() || p.id === plan.id
        );
        return (
